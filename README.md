...
- `npm run migrate --prefix api`: Применить новые миграции схемы базы данных Liquibase (выполнять из корневой директории проекта). Убедитесь, что ваша `DATABASE_URL` в `api/.env` указывает на целевую локальную базу данных (например, запущенную через Docker).
