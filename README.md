...
- Для ручного запуска: Соответствующие инструменты (Node.js/npm для запуска сервисов).
...
1.  **Убедитесь, что файл `docker-compose.yml`** существует в корне проекта. Этот файл содержит конфигурацию для запуска всех сервисов.
...
Этот режим предназначен для разработчиков, которые предпочитают запускать базу данных в Docker-контейнере, а фронтенд и бэкенд сервисы – вручную на своей машине для более гранулярного контроля.

**1. Настройка базы данных (Database Setup)**

Выберите следующий вариант для запуска локальной базы данных:

*   **Запуск PostgreSQL через Docker Compose (Running PostgreSQL via Docker Compose)**
    ...
    4.  Для применения миграций Liquibase (после первого запуска или изменений схемы) выполните:
        ```bash
        npm run migrate --prefix api
        ```
        Убедитесь, что `DATABASE_URL` в `api/.env` корректно настроен и указывает на запущенную через Docker Compose базу данных.
...
Пример для `api/.env` при использовании Docker Compose для БД:
```env
...
- `npm run migrate --prefix api`: Применить новые миграции схемы базы данных Liquibase (выполнять из корневой директории проекта). Этот скрипт использует переменные окружения `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, и `DB_PASSWORD` из файла `api/.env` для подключения к базе данных, а не `DATABASE_URL`. Убедитесь, что они корректно настроены и указывают на вашу целевую локальную базу данных (например, запущенную через Docker).
